...
function Sound(soundPlayer, url, uuid) {
...
Sound.prototype.play = function(callback) {
  ...
  this.soundPlayer.findPlayer(function(error, player) {
    if (error) {
...
Sound.prototype._getStream = function(player, callback) {
  var url = this.url;
  debug("Get stream url=", url, " player=", player);
  ...
  if (/^https:/.exec(url)) {
    ...
    var request = https.get(httpOptions, function(response) {
      debug("Read https stream", url, response, request);
...
Sound.prototype._processProgress = function(d) {
  ...
  if (this._lastProgressDate &&
      this._lastProgressDate + soundPlayer.minimumDelayBetweenProgress > now) {
    return;
  }
