  ...
  function playing(stream, mimeType) {
    ...
    self
        .findPlayer(function(error, playerName, player) {
          ...
          player = child_p.spawn(playerExe, args, {
                  ...
                  function(data) {
                    ...
                    ds
                        .forEach(function(d) {
                          ...
                          var playRegExp = /In:([0-9.]+)% ([0-9]{2}):([0-9]{2}):([0-9]{2}).([0-9]{2,3}) \[([0-9]{2}):([0-9]{2}):([0-9]{2}).([0-9]{2,3})\]/
                              .exec(d);
                          ...
                          if (playRegexp) {
                            ...
                            offset = offset * 60 + parseInt(playRegExp[4], 10);
                            offset = offset + parseInt(playRegExp[5], 10) /
                                ((playRegExp[5].length == 2) ? 100 : 1000);
                            var left = parseInt(playRegExp[6], 10);
                            left = left * 60 + parseInt(playRegExp[7], 10);
                            left = left * 60 + parseInt(playRegExp[8], 10);
                            left = left + parseInt(playRegExp[9], 10) /
                                ((playRegExp[9].length == 2) ? 100 : 1000);

                            debug("Offset=", offset, "left=", left);
                          }
                        });
                  });
