...
if(typeof exports === 'undefined'){
  ...
  */
  ...
  Play.prototype.sound = function sound (file, callback) {
    ...
    child.on('exit', function (code, signal) {
      if(code == null || signal != null || code === 1) {
        console.log('couldnt play, had an error ' + '[code: '+ code + '] ' + '[signal: ' + signal + '] :' + self.playerName.cyan);
  ...
  Play.prototype.stop = function stop (callback) {
    var callback = callback || function () {};

    var self = this;

    exec("killall " + this.playerName, function (err, stdout) {
      if (err) {
        self.
        return;
      }
